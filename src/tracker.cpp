...
Tracker::Tracker(std::string kernel_type, std::string feature_type) {
  /*!
    ...
    param:
      kernel_type: gaussian, poly, linear
      feature_type: hog, fhog, gray, color
    output:
      none
   !*/
  padding_ = 1.5;
...
//------------------------------------------------------------------------------
void Tracker::Init(cv::Mat image, cv::Rect rect_init) {
  /*!
    constructor of tracker
    param:
      kernel_type: gaussian, poly, linear
  ...
  if (resize_image_) {
    pos_ = Tools::FloorPointScale(pos_, 0.5);
    target_sz_ = Tools::FloorSizeScale(target_sz_, 0.5);
  }

  window_sz_ = Tools::FloorSizeScale(target_sz_, 1 + padding_);

  float output_sigma = std::sqrt(float(target_sz_.area())) * output_sigma_factor_ / cell_size_;

  cv::dft(GaussianShapedLabels(output_sigma, Tools::FloorSizeScale(window_sz_, 1. / cell_size_)),
...
//------------------------------------------------------------------------------
void Tracker::Learn(cv::Mat &patch, float lr) {

  std::vector<cv::Mat> x = GetFeatures(patch);

  std::vector<cv::Mat> xf(x.size());
  //do DFT to each channels of feature map x
  for (unsigned int i = 0; i < x.size(); i++)
    cv::dft(x[i], xf[i], DFT_COMPLEX_OUTPUT);
  // get kernal feature with different kernal
  cv::Mat kf;
  if (strcmp(kernel_type_.c_str(), "gaussian") == 0)
...
//------------------------------------------------------------------------------
cv::Mat Tracker::CreateGaussian1D(int n, float sigma) {
...
//------------------------------------------------------------------------------
cv::Mat Tracker::CreateGaussian2D(cv::Size sz, float sigma) {
...
//-----------------------------------------------------------------------------------------------
cv::Mat Tracker::GaussianShapedLabels(float sigma, cv::Size sz) {
...
//------------------------------------------------------------------------------
cv::Mat Tracker::CalculateHann(cv::Size sz) {
...
//--------------------------------------------------------------------------------------------
cv::Mat Tracker::GaussianCorrelation(std::vector<cv::Mat> xf, std::vector<cv::Mat> yf) {
  /*!
...
//--------------------------------------------------------------------------------------------
cv::Mat Tracker::PolynomialCorrelation(std::vector<cv::Mat> xf, std::vector<cv::Mat> yf) {
  std::vector<cv::Mat> xyf_vector(xf.size());
...
//--------------------------------------------------------------------------------------------
cv::Mat Tracker::LinearCorrelation(std::vector<cv::Mat> xf, std::vector<cv::Mat> yf) {
  /*!
      description: linearcorrelation (no kernal)
      intput:
        xf: feature map(vector of mat)
        yf:
      output:
        inner product: mat
   !*/
...
//--------------------------------------------------------------------------------------------
cv::Mat Tracker::ComplexMul(const cv::Mat &x1, const cv::Mat &x2) {
...
//--------------------------------------------------------------------------------------------
cv::Mat Tracker::ComplexDiv(const cv::Mat &x1, const cv::Mat &x2) {
  std::vector<cv::Mat> planes1;
  cv::split(x1, planes1);
  std::vector<cv::Mat> planes2;
  cv::split(x2, planes2);
